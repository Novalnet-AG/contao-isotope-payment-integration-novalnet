...
class NovalnetPaypal extends Postsale implements IsotopePayment
{
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        // Add the paypal cart info
        if (!empty(Isotope::getCart()->getItems())) {
            $arrItems = Isotope::getCart()->getItems();
            foreach ($arrItems as $arrItem) {
                $data['cart_info']['line_items'][] = array( 'name' => $arrItem->getName(), 'price' => round((float) sprintf('%0.2f', $arrItem->getPrice()) * 100), 'quantity' => $arrItem->quantity, 'description' => '', 'category' => 'physical' );
            }

            if (Isotope::getCart()->hasShipping()) {
                $data['cart_info']['items_shipping_price'] = round((float) sprintf('%0.2f', Isotope::getCart()->getRelated('shipping_id')->getPrice()) * 100);
            }
        }
        ...
        if ($this->novalnetpaypalOneClickShopping == 1) {
            ...
            $paymentData = \Database::getInstance()->execute("SELECT payment_data FROM tl_iso_product_collection WHERE member={$order->member} AND payment_id={$paymentID} AND payment_data != '' AND payment_data LIKE '%token%' AND payment_data LIKE '%PAYPAL%' ORDER BY document_number DESC")
            ->fetchAllAssoc();
            ...
            foreach($paymentData as $key => $value) {
                ...
                if (!empty($storedData['token']) && !empty($storedData['payment_type']) && $storedData['payment_type'] === 'PAYPAL') {
