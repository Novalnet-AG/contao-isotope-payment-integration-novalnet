    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        if ($this->novalnetinstalmentsepaOneClickShopping == 1) {
            $paymentData = \Database::getInstance()->execute("SELECT payment_data FROM tl_iso_product_collection WHERE member={$order->member} AND payment_data != '' AND payment_data LIKE '%token%' AND payment_data LIKE '%SEPA%' ORDER BY document_number DESC")
            ->fetchAllAssoc();
            ...
            foreach($paymentData as $key => $value) {
                ...
                if (!empty($storedData['token']) && !empty($storedData['payment_type']) && str_contains($storedData['payment_type'], 'SEPA')) {
        ...
        $minAmount = ($minAmount) ? $minAmount : 3000;
