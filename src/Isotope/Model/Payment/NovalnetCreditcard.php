...
class NovalnetCreditcard extends Postsale implements IsotopePayment
{
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        if (!empty($this->novalnetccEnforce3d)) {
        ...
        if ($this->novalnetccOneClickShopping == 1) {
            ...
            $paymentData = \Database::getInstance()->execute("SELECT payment_data FROM tl_iso_product_collection WHERE member={$order->member} AND payment_id={$paymentID} AND payment_data != '' AND payment_data LIKE '%token%' AND payment_data LIKE '%CREDITCARD%' ORDER BY document_number DESC")
            ->fetchAllAssoc();
            ...
            foreach($paymentData as $key => $value) {
                ...
                if (!empty($storedData['token']) && !empty($storedData['payment_type']) && $storedData['payment_type'] === 'CREDITCARD') {
