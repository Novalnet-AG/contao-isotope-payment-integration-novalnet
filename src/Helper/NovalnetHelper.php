...
{
    public $paymentType = array('novalnetsepa' => 'DIRECT_DEBIT_SEPA', 'novalnetcc' => 'CREDITCARD', 'novalnetinvoice' => 'INVOICE','novalnetprepayment' => 'PREPAYMENT','novalnetguaranteedinvoice' => 'GUARANTEED_INVOICE', 'novalnetguaranteedsepa' => 'GUARANTEED_DIRECT_DEBIT_SEPA', 'novalnetideal' => 'IDEAL', 'novalnetonlinebanktransfer' => 'ONLINE_BANK_TRANSFER', 'novalnetprzelewy24' => 'PRZELEWY24', 'novalneteps' => 'EPS', 'novalnetinstalmentinvoice' => 'INSTALMENT_INVOICE', 'novalnetinstalmentsepa' => 'INSTALMENT_DIRECT_DEBIT_SEPA', 'novalnetpaypal' => 'PAYPAL', 'novalnetpostfinancecard' => 'POSTFINANCE_CARD', 'novalnetpostfinance' => 'POSTFINANCE', 'novalnetbancontact' => 'BANCONTACT', 'novalnetmultibanco' => 'MULTIBANCO', 'novalnettwint' => 'TWINT');
            ...
            if (hash_equals($generatedChecksum, $checkSum) == false) {
                ...
                if ($transactionData['result']['status'] == 'SUCCESS') {
                    ...
                    $paymentData = array(
                     ...
                     'paid_amount' => in_array($transactionData['transaction']['payment_type'], array('INVOICE', 'PREPAYMENT', 'MULTIBANCO')) ? '0' : $transactionData['transaction']['amount'],
                    );
                    ...
                    if (in_array($transactionData['transaction']['payment_type'], array('PREPAYMENT', 'MULTIBANCO'))  || $transactionData['transaction']['status'] == 'PENDING') {
                        $orderStatus = !empty($this->getNovalnetGlobalConfig('novalnetglobalconfigPendingOrderStatus')->novalnetglobalconfigPendingOrderStatus) ? $this->getNovalnetGlobalConfig('novalnetglobalconfigPendingOrderStatus')->novalnetglobalconfigPendingOrderStatus : '1';
                    } elseif ($transactionData['transaction']['status'] == 'ON_HOLD') {
                        $orderStatus = !empty($this->getNovalnetGlobalConfig('novalnetglobalconfigHoldOrderStatus')->novalnetglobalconfigHoldOrderStatus) ? $this->getNovalnetGlobalConfig('novalnetglobalconfigHoldOrderStatus')->novalnetglobalconfigHoldOrderStatus : '4';
                    } else {
    ...
    public function sendNotification($order, $currentNotification)
    {
        ...
        if (\count($notificationIds) > 0) {
            ...
            \System::log('No notification for order ID ' . $order->id, __METHOD__, TL_ERROR);
        }
    }
     ...
     * @param int $orderNo
     * @param bool $checkoutInfo
     * @return string
     */
    public function prepareComments($response, $orderNo, $checkoutInfo = true)
    {
        $comments = '';
        if (in_array($response['transaction']['payment_type'], array('GUARANTEED_DIRECT_DEBIT_SEPA', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA'))) {
            if ($response['transaction']['status'] == 'PENDING') {
                $comments .= in_array($response['transaction']['payment_type'], array('GUARANTEED_INVOICE', 'INSTALMENT_INVOICE'))
                    ? PHP_EOL . specialchars($GLOBALS['TL_LANG']['MSC']['nn_invoice_guarantee_comment']) . PHP_EOL
                    : PHP_EOL . specialchars($GLOBALS['TL_LANG']['MSC']['nn_sepa_guarantee_comment']) . PHP_EOL;
            }
        }
        ...
        if (in_array($response['transaction']['payment_type'], array('INVOICE', 'PREPAYMENT')) || (in_array($response['transaction']['payment_type'], array('INSTALMENT_INVOICE', 'GUARANTEED_INVOICE')) && $response['transaction']['status'] != 'PENDING')) {
            ...
            if ($checkoutInfo && !empty($response['transaction']['bank_details']['qr_image'])) {
                $comments .= PHP_EOL . PHP_EOL . specialchars($GLOBALS['TL_LANG']['MSC']['nn_payment_qr_code_reference']) . PHP_EOL . '<img src="' . $response['transaction']['bank_details']['qr_image']. '"/>' . PHP_EOL;
            }
    ...
    public function prepareInvoiceComments($response, $orderNo)
    {
        ...
        if (in_array($response['transaction']['status'], array('CONFIRMED', 'PENDING')) && !empty($response['transaction']['due_date'])) {
            $comments =  sprintf(specialchars($GLOBALS['TL_LANG']['MSC']['nn_order_note_due_date']), sprintf('%0.2f', $response['transaction']['amount'] / 100).' '. $response['transaction']['currency'], date('d.m.Y', strtotime($response['transaction']['due_date']))). PHP_EOL . PHP_EOL;
        } else {
            $comments =  sprintf(specialchars($GLOBALS['TL_LANG']['MSC']['nn_order_note']), sprintf('%0.2f', $response['transaction']['amount'] / 100).' '. $response['transaction']['currency']). PHP_EOL . PHP_EOL;
        }
        ...
        $comments .= specialchars($GLOBALS['TL_LANG']['MSC']['nn_bank_place']). $response['transaction']['bank_details']['bank_place'] . PHP_EOL . PHP_EOL;
