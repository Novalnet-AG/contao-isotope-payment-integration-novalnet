...
class NovalnetWebHook
{
    ...
    */
    public function getOrderReference($order)
    {
        ...
        } elseif ($this->eventType === 'PAYMENT' && empty($paymentData->payment_data) && $this->eventData['result']['status'] == 'SUCCESS') { // Handle the communication break
    ...
    public function handleCommunicationFailure($order)
    {
        ...
        $paymentData = array(
             ...
             'paid_amount' => in_array($this->eventData['transaction']['payment_type'], array('INVOICE', 'PREPAYMENT', 'CASHPAYMENT', 'MULTIBANCO')) ? '0' : $this->eventData['transaction']['amount']
        );
        ...
        if (in_array($this->eventData['transaction']['status'], array('PENDING', 'ON_HOLD', 'CONFIRMED', 'DEACTIVATED'))) {
            if (in_array($this->eventData['transaction']['update_type'], ['DUE_DATE', 'AMOUNT_DUE_DATE'])) {
                ...
                if ($this->eventData['transaction']['payment_type'] === 'CASHPAYMENT') {
                    $comments = PHP_EOL . sprintf(specialchars($GLOBALS['TL_LANG']['MSC']['nn_webhook_cashpayment_transfer_update']), $this->receivedAmount.' ' .$this->eventData['transaction']['currency'], $dueDate).PHP_EOL;
                } else {
                ...
                if ($this->eventData['transaction']['status']  == 'DEACTIVATED') {
                    ...
                    if (in_array($this->eventData['transaction']['payment_type'], ['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'])) {
                        $comments = $this->helper->prepareComments($this->eventData, $this->orderId, 'guarantee') . PHP_EOL . PHP_EOL;
                    }
                    ...
                    if (in_array($this->orderReference['payment_type'], array('INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA')) && isset($this->eventData['instalment'])) {
                        //instalment comments
                        $comments .= $this->helper->prepareComments($this->eventData, $this->orderId) . PHP_EOL . PHP_EOL;
                        $this->orderReference['instalment_details'] = $this->helper->getInstalmentData($this->eventData);
                    }
            ...
            if (in_array($this->eventData['transaction']['payment_type'], array('INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT' ))) {
                ...
                if ($amountAlreadyPaid < $this->orderReference['amount']) {
                    ...
                    if (((int) $totalPaidAmount >= (int) $this->orderReference['amount'])) {
                        ...
                        $order->updateOrderStatus($callbackOrderStatus->$statusStatus ?: $this->orderStatus);
                    }
